...
- 이미 실행 중인 일반 사용자 profile은 Playwright가 안전하게 제어할 수 없다. DOM 자동 조작이 필요하면 사용자 browser에 설치된 `extensions/user-browser-bridge`의 명시적 레시피 지원 여부를 먼저 확인한다. 해당 확장이 없거나 지원하지 않는 작업만 전용 제어 session 사용 여부를 사용자에게 명확히 알리고 허용을 받는다.
- 일반 사용자 browser 연결 확장은 사용자가 직접 허용한 사이트 origin에서 저장소에 포함된 레시피별 명령만 실행한다. 같은 목적 탭 또는 빈 탭을 우선 재사용하고, 재생·업로드·입력 중인 탭은 교체하지 않는다.
- 사용자가 `레시피 입력 시작`을 요청하면 `recipes:input:start`으로 로컬 입력 상태를 시작하고, 이후 제공하는 이름·URL·단계·검증 결과를 `recipes:input:add`으로 누적한다. `레시피 입력 종료`는 기본 Git 제외 `recipes/local/` 저장이며, 공개 저장은 사용자가 명시적으로 요청한 경우에만 `--publish`로 수행한다.
- 레시피 입력에는 비밀번호, OTP, cookie, 연결 키, 결제 정보, 개인 식별 정보를 기록하지 않는다. 등록된 workflow와 연결되지 않는 기록은 `draft`로 보관한다.
